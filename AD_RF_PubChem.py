...
with open("/home/dell/wuchao/ML/1785_IARC/1697FS/model/0106/RF_PubChem_1697_FS.pkl", 'rb') as mymodel:
...
for code in ['rigid','exp']:
    ...
    dfF1 = pd.DataFrame(index=IAVal_List,columns=ρsDict[code])
    dfPrecision = pd.DataFrame(index=IAVal_List,columns=ρsDict[code])
    ...
    for densLB in dfAUC.columns:
        for LdUB in dfAUC.index:
            ...
            dfF1.loc[LdUB,densLB] = metrics.f1_score(y_tr[adi],y_pred[adi])  # Calculate F1 score
            dfPrecision.loc[LdUB,densLB] = metrics.precision_score(y_tr[adi],y_pred[adi]) 
    ...
    dfF1.to_csv('/home/dell/wuchao/ML/1785_IARC/1697FS/AD/1697ADResults/56_85_Classifier{:s}_AD_F1_{}.csv'.format(code,save_time))  # Save F1 scores
    dfPrecision.to_csv('/home/dell/wuchao/ML/1785_IARC/1697FS/AD/1697ADResults/56_85_Classifier{:s}_AD_Precision_{}.csv'.format(code,save_time))